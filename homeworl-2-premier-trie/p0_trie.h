...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node)noexcept {
    TrieNode node = TrieNode(other_trie_node.key_char_);
    std::unordered_map<char, std::unique_ptr<TrieNode>>::iterator pr;
    for (pr = other_trie_node.children_.begin(); pr != other_trie_node.children_.end(); pr++)
      node.InsertChildNode(pr->first,std::move(pr->second));
  }
  ...
  bool HasChild(char key_char) const {
    std::unordered_map<char, std::unique_ptr<TrieNode>>::const_iterator pr;
    for (pr = children_.begin(); pr != children_.end(); pr++)
      if (pr->first == key_char){
        return true;
      }
    return false;
  }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for(i = key.begin();i != key.end();i++){
      std::unique_ptr<TrieNode> * child = node->GetChildNode(*i);
      if (counter == key.length()-1){
        if (child){
          if (child->get()->IsEndNode()){
            ...
            this->latch_.WLock();
            std::unique_ptr<TrieNodeWithValue<T>> converted_node =
                std::unique_ptr<TrieNodeWithValue<T>>(new TrieNodeWithValue<T>(std::move(**child), value));
            node->RemoveChildNode(*i);
            node->InsertChildNode(*i,std::move(converted_node));
            this->latch_.WUnlock();
            return true;
          }
        } else {
          this->latch_.WLock();
          std::unique_ptr<TrieNodeWithValue<T>> converted_node =std::unique_ptr<TrieNodeWithValue<T>>(new TrieNodeWithValue<T>(*i, value));
          node->InsertChildNode(*i,std::move(converted_node) );
          this->latch_.WUnlock();
          return true;
        }
        ...
        if (child){
          ...
          this->latch_.WLock();
          node->InsertChildNode(*i, std::make_unique<TrieNode>(*i));
          this->latch_.WUnlock();
          node = node->GetChildNode(*i)->get();
        }
      }
      counter++;
    }
  ...
  bool DoRemove(const std::string &key, bool isFullKey){
    if (key == ""){
      return false;
    }
    TrieNode* node = root_.get();
    ...
    for(i = key.begin();i != key.end();i++){
      auto uniqueNode = node->GetChildNode(*i);
      if ( uniqueNode== nullptr){
        return false;
      }
      if (key.end() - i == 1){
        if (isFullKey){
          if ( !uniqueNode->get()->IsEndNode()){
            return false;
          }  else if ( uniqueNode->get()->HasChildren()){
            return true;
          } else {
            latch_.WLock();
            node->RemoveChildNode(*i);
            latch_.WUnlock();
            return DoRemove(key.substr(0,key.length()-1),false);
          }

        } else {
          if (uniqueNode->get()->HasChildren() || uniqueNode->get()->IsEndNode()){
            return true;
          }
        }
      }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    for(i = key.begin();i != key.end();i++){
      ...
      if ( key.end() - i == 1 ){
        TrieNodeWithValue<T> * v = dynamic_cast<TrieNodeWithValue<T> *>(uniqueNode->get());
        if (v){
          if (!v->IsEndNode()){
            *success = false;
            return T();
